...
type ProfessorCourse = {
  ...
  interface HomeClientPageProps {
    ...
    initialPinnedCourses: CourseNew[];
    initialPinnedCourseIds: number[];
  ...
  export default function HomeClientPage({
    initialUser,
    initialUserRole,
    initialPinnedCourses,
    initialPinnedCourseIds,
    initialCoursesData,
    ...
    const [courses, setCourses] = useState(initialCoursesData.courses);
    const totalCourses=initialCoursesData.total
    const [pinnedCourses, setPinnedCourses] = useState(initialPinnedCourses);
    const [pinnedCourseIds, setPinnedCourseIds] = useState(new Set(initialPinnedCourseIds));
    const [professorCourses, setProfessorCourses] = useState<GroupedProfessorCourses[]>([]); // Will be populated by fetch
    const [totalProfessorEntries, setTotalProfessorEntries] = useState(initialProfessorData.total);
    const [initialFetch, setInitialFetch] = useState(false);
    // --- State that remains purely client-side ---
    ...
    const roleLoading=false // No longer needed for initial load
    ...
    const CourseSelectUpload = dynamic(() => import('@/components/courseselectupload'), { ssr: false })
    ...
    useEffect(() => {
        // This effect remains to handle real-time auth changes (e.g., login/logout on another tab)
        const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
          // You could simplify this to just refresh the page to get new server props
          // or handle the state update dynamically on the client.
          if (session?.user?.id !== user?.id) {
             router.refresh(); // Easiest way to get fresh server props
          }
        });
    ...
    {/* {user && roleLoading && (
      ...
      <div className="h-8 w-[60px] sm:h-10 sm:w-[110px] rounded-md" />
          ...
          <SearchBox courses={allCourses} open={open} setOpen={setOpen} />
    
          {/* Course Selection Combobox for Upload Content */}
          {courseComboboxOpen && (
            <CourseSelectUpload courses={allCourses} open={courseComboboxOpen} setOpen={setCourseComboboxOpen} />
           )}
        </div>
      );
