...
/**
 * One panel treatment on the shadcn Drawer: a swipe-dismissable bottom sheet on
 * phones, a floating card bottom-left on desktop. `noBodyStyles` keeps vaul from
 * pinning the body — the map page manages its own scroll.
 */
  ...
  return (
    <Drawer open onOpenChange={(next) => !next && onClose()} noBodyStyles>
      <DrawerContent
        ...
        className={`${SHEET} z-[1200] max-h-[86svh] overflow-hidden rounded-t-[26px] sm:inset-x-auto sm:bottom-4 sm:left-4 sm:mt-0 sm:max-h-[calc(100svh-2rem)] sm:w-[384px] sm:rounded-[26px]`}
      >
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="absolute right-2.5 top-2.5 z-10 grid size-8 place-items-center rounded-full text-muted-teal-500 transition hover:bg-black/5 hover:text-muted-teal-900 dark:hover:bg-white/10 dark:hover:text-white"
          >
            <X className="size-4" />
          </button>
        </div>
