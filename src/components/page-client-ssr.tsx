...
const getSemesterDisplay = (semesterNumber: number) => `Sem ${semesterNumber}`

function buildEnhancedFromRows(
  content: (Course_content_anon | Course_content_user)[],
  professors: Professor[],
  tags: Tag[],
): EnhancedContent[] {
  if (content.length === 0 || professors.length === 0) return []
  return content.map((item: Course_content_anon | Course_content_user) => {
    const professor = professors.find((p) => p.id === item.professor_id)
    const itemTags =
      item.tag_ids && tags.length > 0
        ? tags
            .filter((tag: Tag) => item.tag_ids!.includes(tag.id))
            .map((tag) => tag.name)
        : []
...
function availableTagsFromEnhanced(enhanced: EnhancedContent[]): string[] {
...
export default function CourseViewPage({
  ...
  const [enhancedContent, setEnhancedContent] = useState<EnhancedContent[]>(() =>
    buildEnhancedFromRows(serverContent ?? [], serverProfessors ?? [], serverTags ?? []),
  )
  const [tags, setTags] = useState<Tag[]>(serverTags || [])
  const [search, setSearch] = useState("")
  const [selectedTags, setSelectedTags] = useState<string[]>([])
  const [availableTags, setAvailableTags] = useState<string[]>(() =>
    availableTagsFromEnhanced(
      buildEnhancedFromRows(serverContent ?? [], serverProfessors ?? [], serverTags ?? []),
    ),
  )
  ...
  const [isContentReady, setIsContentReady] = useState(() => {
    const initial = buildEnhancedFromRows(serverContent ?? [], serverProfessors ?? [], serverTags ?? [])
    return Boolean(serverCourse && initial.length > 0)
  })
  ...
  // Initialize enhanced content from server props / client refetches
  useEffect(() => {
    if (content.length > 0 && professors.length > 0) {
      const enhanced = buildEnhancedFromRows(content, professors, tags)
      setEnhancedContent(enhanced)
      setAvailableTags(availableTagsFromEnhanced(enhanced))
    }
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        // Fetch course content from Spring Boot API instead of Supabase view (skip when SSR already supplied rows)
        const courseNumericId = Number(courseId)
        if (!skipInitialContentFetch && !Number.isNaN(courseNumericId)) {
          try {
